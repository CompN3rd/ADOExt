...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // src/views/webview/richText.ts
  var AdoRichText = class extends i4 {
    constructor() {
      super(...arguments);
      this.htmlText = "";
    ...
    render() {
      if (!this.htmlText && this.emptyLabel) {
        return b2`<em class="empty">${this.emptyLabel}</em>`;
      }
      return A;
    }
    updated() {
  ...
  var GLOBAL_ATTRS = /* @__PURE__ */ new Set(["class"]);
  var TAG_ATTRS = {
    ...
    th: /* @__PURE__ */ new Set(["colspan", "rowspan", "scope", "align"]),
    ol: /* @__PURE__ */ new Set(["type", "start"])
  };
  function renderSanitizedHtml(rawHtml, container) {
  ...
  // src/views/webview/workItemDetails.ts
  var AdoWorkItemDetailsApp = class extends i4 {
    constructor() {
      super(...arguments);
    ...
    render() {
      ...
      return b2`<main class="shell" style=${style}>
            <div class="toolbar">
                ...
                <div class="state-edit"><select aria-label="Work item state" .value=${this.selectedState} @change=${this.onStateChanged}>${this.data.allowedStates.map((state) => b2`<option value=${state}>${state}</option>`)}</select><button class="btn-primary" @click=${this.updateState}>Update State</button></div>
            </div>
            <h1>${this.data.workItemTypeIconUrl ? b2`<img class="type-icon" src=${this.data.workItemTypeIconUrl} alt="" />` : A}<span class="badge badge-type">${this.data.workItemType}</span><span class="badge badge-state">${this.data.state}</span>${this.data.priority !== void 0 ? b2`<span class="badge priority-${this.data.priority}">P${this.data.priority}</span>` : A}#${this.data.id}: ${this.data.title}</h1>
            <section class="section"><table class="meta-table">${this.data.metaRows.map((row) => b2`<tr><td class="meta-label">${row.label}</td><td>${row.value}</td></tr>`)}</table></section>
    ...
    renderLinkedItems() {
      if (this.data.linkedItems.length === 0) {
        return b2`<p class="empty">No linked branches, commits, or pull requests.</p>`;
      }
      return b2`${this.data.linkedItems.map((item) => b2`<button class="btn-secondary linked-item-btn" @click=${() => this.openLinkedItem(item)}>${this.linkedIcon(item.type)} ${item.label}</button>`)}`;
    }
    renderComments() {
      if (this.data.comments.length === 0) {
        return b2`<p class="empty">No comments yet.</p>`;
      }
      return b2`${this.data.comments.map((comment) => b2`<article class="comment"><div class="comment-header"><span class="comment-author">${comment.author}</span><span class="comment-date">${comment.date}</span></div><ado-rich-text .htmlText=${comment.html} .plainText=${comment.isPlainText}></ado-rich-text></article>`)}`;
    }
    linkedIcon(type) {
      switch (type) {
        case "pr":
  ...
  AdoWorkItemDetailsApp.properties = {
    data: { state: true },
    selectedState: { state: true }
  };
  AdoWorkItemDetailsApp.styles = i`
        :host { display: block; }
