      ...
      return b2`<main class="shell">
            ...
            <section class="section">
                ...
                ${this.data.artifacts.length === 0 ? b2`<p class="empty">No artifacts found.</p>` : b2`<div class="artifacts">
                        ${this.data.artifacts.map((a3) => b2`<div class="artifact">
                            <span class="artifact-name" title=${a3.name}>${a3.name}</span>
                            ${a3.downloadUrl ? b2`<button class="btn-secondary" @click=${() => this.send({ type: "openArtifact", url: a3.downloadUrl })}>Open</button>` : b2`<button class="btn-secondary" disabled>Open</button>`}
                        </div>`)}
                    </div>`}
            </section>
        </main>`;
