...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // src/views/webview/pipelineRunDetails.ts
  var AdoPipelineRunDetailsApp = class extends i4 {
    constructor() {
      super(...arguments);
    ...
    render() {
      ...
      return b2`<main class="shell">
            ...
            <div class="meta">
                ${subtitleParts.join(" \xB7 ")}
                ${this.data.repository ? b2`<div>Repo: <code>${this.data.repository}</code></div>` : A}
                ${this.data.commit ? b2`<div>Commit: <code>${this.data.commit}</code></div>` : A}
                ${this.data.yamlFile ? b2`<div>YAML: <code>${this.data.yamlFile}</code></div>` : A}
                ${this.data.startTime ? b2`<div>Started: ${this.data.startTime}${this.data.finishTime ? b2` · Finished: ${this.data.finishTime}` : A}</div>` : A}
            </div>
            ...
            <section class="section">
                ...
                ${this.data.artifacts.length === 0 ? b2`<p class="empty">No artifacts found.</p>` : b2`<div class="artifacts">
                        ${this.data.artifacts.map((artifact) => b2`<div class="artifact">
                            <span class="artifact-name" title=${artifact.name}>${artifact.name}</span>
                            ${artifact.downloadUrl ? b2`<button class="btn-secondary" @click=${() => this.send({ type: "openArtifact", url: artifact.downloadUrl })}>Open</button>` : b2`<button class="btn-secondary" disabled>Open</button>`}
                        </div>`)}
                    </div>`}
            </section>
        </main>`;
    ...
    renderTimeline(nodes) {
      return b2`<ul class="timeline">
            ${nodes.map((node) => b2`<li>
                <div class="node">
                    <span class="badge ${this.badgeClass(node.statusKind)}">${node.statusLabel || node.statusKind}</span>
                    <span>${node.name}</span>
                    ${node.duration ? b2`<span class="meta">${node.duration}</span>` : A}
                    ${node.logId ? b2`<button class="btn-link" title="Open step log" @click=${() => this.send({ type: "openStepLog", logId: node.logId, stepName: node.name })}>Log</button>` : A}
                </div>
                ${node.children.length > 0 ? this.renderTimeline(node.children) : A}
            </li>`)}
        </ul>`;
    }
    badgeClass(kind) {
  ...
  AdoPipelineRunDetailsApp.properties = {
    data: { state: true }
  };
  AdoPipelineRunDetailsApp.styles = i`
        :host { display: block; }
