...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var AdoPrDetailsApp = class extends i4 {
    ...
    renderThreads() {
      return this.data.threads.length === 0 ? b2`<p class="empty">No comment threads.</p>` : b2`${this.data.threads.map((thread) => this.renderThread(thread))}`;
    }
    renderThread(thread) {
      return b2`<article class="thread ${thread.isResolved ? "resolved" : ""}">
            ...
            ${thread.comments.map((comment) => b2`<div class="comment"><div class="comment-author">${comment.author}</div><div class="comment-content">${comment.content}</div></div>`)}
