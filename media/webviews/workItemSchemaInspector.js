...
`;
  ...
  var AdoWorkItemSchemaInspectorApp = class extends i4 {
      ...
      return b2`<main class="shell">
            <div class="header">
                <div class="title">
                    ...
                    <div class="subtitle">${this.model.organization}/${this.model.project}${processLabel ? b2` · ${processLabel}` : A}</div>
            ...
            ${this.model.warnings.length ? b2`<section class="warnings"><h2>Warnings</h2><ul>${this.model.warnings.map((w2) => b2`<li>${w2}</li>`)}</ul></section>` : A}
            ...
            ${filtered.length === 0 ? b2`<p class="empty">No matching work item types.</p>` : b2`${filtered.map((type) => this.renderType(type))}`}
    ...
    renderType(type) {
      ...
      return b2`<details>
            <summary style=${style}>
                ${type.iconUrl ? b2`<img class="type-icon" src=${type.iconUrl} alt="" />` : A}
                <span class="type-color" aria-hidden="true"></span>
                <span class="type-name">${type.name}</span>
                <span class="type-meta">${type.referenceName ? b2`<code>${type.referenceName}</code> · ` : A}<span class="pill">${type.stateCount} states</span> <span class="pill">${type.fieldCount} fields</span></span>
            </summary>
            <div class="body">
                <div class="grid">
                    <section>
                        <h2>States</h2>
                        ${type.states.length === 0 ? b2`<p class="empty">No state metadata available.</p>` : b2`<table><thead><tr><th>Name</th><th>Category</th></tr></thead><tbody>${type.states.map((state) => this.renderStateRow(state))}</tbody></table>`}
                    </section>
                    <section>
                        <h2>Fields</h2>
                        ${type.fields.length === 0 ? b2`<p class="empty">No field metadata available.</p>` : b2`<table><thead><tr><th>Reference</th><th>Name</th><th></th></tr></thead><tbody>${type.fields.map((field) => this.renderFieldRow(field))}</tbody></table>`}
                    </section>
                </div>
            </div>
        </details>`;
    ...
    renderFieldRow(field) {
      return b2`<tr>
            <td><code>${field.referenceName}</code>${field.alwaysRequired ? b2` <span class="pill">required</span>` : A}</td>
            <td>${field.name}${field.helpText ? b2`<div class="meta">${field.helpText}</div>` : A}</td>
